// File: /Users/puvansivanasan/perplexity/jobs/active/2026-001-RPR-WEBSITE/src/services/ollieclient.ts

/**
 * TS-Λ3 // OLLIE CLIENT SERVICE [v3.6.0]
 * Path: src/services/ollieclient.ts
 * Persona: Bill Burr Cynical Latch (useful but no-nonsense)
 * Mission: Internal Bridge Latch + hardened language detection for regional locales (en-US, zh-CN).
 ...
 * Traffic is proxied by Firebase Hosting:
 *   /api/ollieChat  -> Cloud Run service "olliechat" (asia-southeast1)
 */
const OLLIE_ENDPOINT = "/api/ollieChat";
...
export async function askOllie(prompt: string, locale: string = "en"): Promise<string> {
    ...
    try {
        const res = await fetch(OLLIE_ENDPOINT, {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                ...
                context:
                    "ACT AS OLLIE: A dry, cynical, slightly sarcastic but highly useful AI concierge. You are over the corporate fluff. You speak the requested language fluently but keep the attitude. You hate waste and 'curious chatbots'—be useful or be quiet.",
            }),
        });
